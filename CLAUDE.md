...
- **Botones en card-header oscuro** (solo en `.card-zymos` legacy): `.card-zymos .card-header` usa gradiente oscuro. Botones dentro DEBEN usar `btn-outline-crema`. NUNCA `btn-zymos-outline` ahí. **Excepción — `dashboard.html`**: aunque usa `.card-zymos` en Planificación y Finanzas, sus botones ya fueron migrados a `btn-outline-secondary` / `btn-primary` para consistencia visual con el resto del dashboard moderno.
...
├── login.html, dashboard.html (personalizable — 6 secciones drag-sortable: Stats Lotes, Inventario, Alertas, Próximas Elaboraciones, Gráficas, Finanzas; stat-cards clickeables con `.stat-card-link`; colores de tabla y Chart.js leen CSS vars en runtime; botón "Reporte" en Finanzas; card "Próximas Elaboraciones" tiene botón "Ver calendario" en su card-header oscuro → usa `btn-outline-secondary btn-sm`), calendario.html, busqueda.html
