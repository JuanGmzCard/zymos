...
├── login.html, dashboard.html (personalizable — 6 secciones drag-sortable: Stats Lotes, Inventario, Alertas, Próximas Elaboraciones, Gráficas, Finanzas; stat-cards clickeables con `.stat-card-link`; colores de tabla y Chart.js leen CSS vars en runtime; botón "Reporte" en Finanzas; card "Próximas Elaboraciones" tiene botón "Ver calendario" en su card-header oscuro → usa `btn-outline-crema btn-sm`), calendario.html, busqueda.html
