...
El selector en `navbar.html` usa `th:href="${#request.requestURI + '?lang=es'}"` — Thymeleaf inyecta la ruta actual (ej: `/inventario?lang=es`). El navegador navega via GET normal, `LocaleChangeInterceptor` procesa el param `lang`, guarda la cookie y la página se recarga en el nuevo idioma de forma inmediata. No requiere JS.
