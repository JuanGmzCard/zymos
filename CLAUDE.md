...
- **JS estático de trazabilidad** (`static/js/`): `<script th:inline="javascript">` inyecta variables globales; el `.js` externo las lee. Archivos:
  - `trazabilidad-ingredientes.js` — wizard de tabs, filas dinámicas, carga de receta (`_recetaPendiente`), stock via `/suggest-items-por-nombre`, `UNIT_OPTIONS_CLAR` (incluye "und" para clarificantes), `goTab()` gestiona clase `done` en `.wz-tab`. Usado por `formulario.html`.
