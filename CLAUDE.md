...
└── admin/      logs.html, tenants.html (lista de tenants con cards + franja de colores + botón "Limpiar cache" → `POST /admin/tenants/cache/evict` + botones por card: "Usuarios" → `/admin/tenants/{subdomain}/usuarios`, "Métricas" → `/admin/tenants/{subdomain}/metricas`, "Migración" → `/admin/migracion/{subdomain}`),
...
- `InsumoInventarioServiceTest`, `TrazabilidadServiceTest` (10 tests), `DashboardServiceTest`
