...
- Flyway: `baseline-on-migrate=true`, migraciones en `db/migration/` (V1–V49). En producción usa credenciales separadas: `FLYWAY_USERNAME=zymos_flyway` / `FLYWAY_PASSWORD` (rol con DDL); si no se definen, usa `DB_USERNAME`/`DB_PASSWORD` como fallback. Ver `db_security.sql` para crear los roles.
...
- `TenantAdminControllerTest` — 8 tests: (1) `lista_sinAuth_retorna401`; (2) `lista_conAdmin_retorna200`; (3) `nuevo_conAdmin_retornaFormulario`; (4) `config_retornaJson` — stubea `buscarPorSubdomain("mosto")` → `Optional.of(tenant)` con todos los campos de color seteados; (5) `formularioEditar_noExiste_redirige` — GET /admin/tenants/editar/noexiste → 3xx redirect a /admin/tenants + flash `tipoMensaje=danger`; (6) `historial_noExiste_redirige` — GET /admin/tenants/noexiste/historial → idem; (7) `usuarios_noExiste_redirige` — GET /admin/tenants/noexiste/usuarios → idem; (8) `config_noExiste_retorna404` — GET /admin/tenants/noexiste/config → HTTP 404 (porque `getConfig` retorna `ResponseEntity`). Requiere `@MockBean PasswordEncoder` (inyectado en constructor del controller). **CRÍTICO**: NO agregar `@MockBean ObjectMapper` — mockear Jackson rompe la autoconfiguración de Spring (`routerFunctionMapping` falla al crear porque `objectMapper.reader()` retorna null en el mock)
...
Ejecutar: `mvn test` (requiere Docker Desktop corriendo con daemon TCP habilitado) — 463 tests, BUILD SUCCESS
