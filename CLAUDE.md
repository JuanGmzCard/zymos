...
- **Navbar**: `sec:authorize` oculta links según rol. Los ítems están agrupados en dropdowns: **Producción** (todos los roles): Trazabilidad, Kanban, Planificación, Comparativa, Calendario; **Almacén** (ADMIN/INVENTARIO): Inventario, Recetas; **Comercial** (ADMIN/FACTURACION): Facturas, Proveedores; **Admin** (ADMIN): dropdown con 3 secciones etiquetadas — *Gestión* (Reportes, Usuarios, Tipos de Cerveza), *Sistema* (Notificaciones, Log de Accesos), *Plataforma* (Tenants — solo SUPERADMIN). Equipos queda como ítem standalone (ADMIN/EQUIPOS). El botón `+` muestra acciones rápidas de creación filtradas por rol. El dropdown de usuario muestra nombre, badge de rol y link a `/perfil/password`.
