    ...
    cols.forEach(function (col) {
        Sortable.create(col, {
            ...
            onEnd: function (evt) {
                ...
                if (targetFase === 'fermentacion' && evt.item.dataset.tieneFermentador !== 'true') {
                    revertirMovimiento(evt.item, evt.from, evt.oldIndex);
                    mostrarToastFermentador(evt.item.dataset.loteId);
                    return;
                }
...
function mostrarToastFermentador(loteId) {
    var container = document.getElementById('kanban-toast-container');
    var t = document.createElement('div');
    t.className = 'kanban-toast danger';
    t.style.maxWidth = '320px';
    t.style.pointerEvents = 'auto';
    t.innerHTML = '<i class="bi bi-exclamation-triangle me-1"></i>Sin fermentador asignado. '
        + '<a href="/editar/' + loteId + '" style="color:#ffc; text-decoration:underline;">Editá el lote</a> para asignarlo primero.';
    container.appendChild(t);
    requestAnimationFrame(function () {
        requestAnimationFrame(function () { t.classList.add('show'); });
    });
